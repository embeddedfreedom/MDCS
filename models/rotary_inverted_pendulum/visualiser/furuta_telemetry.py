...
# --- CONFIGURATION ---
CHANNELS = 6         # Updated: Theta, Alpha, T_dot, A_dot, Torque, Cmd_V
FS = 1000            
HISTORY_SEC = 600    
VIEW_SEC = 10        
...
class TelemetryDash(QtWidgets.QMainWindow):
    def __init__(self, port):
        ...
        # Updated Labels for the new application
        self.labels = ["Theta", "Alpha", "T_dot", "A_dot", "Torque", "Cmd_V"]
        self.colors = ['#00FFFF', '#FF00FF', '#FF4500', '#00FF00', '#FFFF00', '#FFFFFF']
    ...
    def init_ui(self):
        ...
        self.stats_label.setStyleSheet("color: #AAAAAA; font-family: monospace; font-weight: bold;")
        ...
        for i, label in enumerate(self.labels):
            ...
            # Default: show Theta, Alpha, and Torque to keep graph clean
            cb.setChecked(True if i in [0, 1, 4] else False) 
    ...
    def update_buffers(self, data):
        ...
        for i in range(CHANNELS):
            val = data[i]
            # Convert first 4 channels (Theta, Alpha, T_dot, A_dot) if toggled
            if self.use_degrees and i < 4:
