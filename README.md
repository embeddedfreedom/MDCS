...
![](/home/dipto/.var/app/com.github.marktext.marktext/config/marktext/images/2026-02-10-17-22-24-general_architecture.drawio.png)
...
- Update the serial port strings in the **Python files** (`plotter.py` and `visualizer_3d.py`).
...
```
python3 visualiser/plotter.py (Names will differ based on model)
```
...
- **Circular History Buffer:** The plotter allocates a fixed block of memory for exactly **10 minutes** (600,000 samples at 1kHz).
...
2. Ensure your data is formatted as `<val1 val2 val3 val4>\n`.

3. Launch the script and specify your COM/Serial port.

4. Use the **Clear Buffer** button between test runs to reset the timeline.
...
- **Data Format:** Arduino 6-set packet (Radians). Value 1 = Arm, Value 2 = Pendulum.
